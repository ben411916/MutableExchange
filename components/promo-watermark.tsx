...
export default function PromoWatermark() {
  ...
  return (
    <Link
      ...
      className={`fixed top-2 left-2 z-50 flex items-center gap-2 px-3 py-1.5 rounded-md transition-all duration-300 ${
    ...
    >
      <div className="relative">
        <Image src="/images/mutable-token.png" alt="MUTB Token" width={24} height={24} className="rounded-full" />
        {isHovered && (
          <Sparkles
            className="absolute -top-1 -right-1 h-3 w-3 text-yellow-300"
      ...
      <Sparkles className="h-4 w-4 text-yellow-300" style={{ filter: "drop-shadow(0 0 2px #fff)" }} />
    </Link>
  )
}
